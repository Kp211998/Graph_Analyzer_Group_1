...
#     for view in st.session_state['views']:
#         if view['view_name'] == view_name:
#             st.write("in if statement")
#             if 'properties' not in view:
#                 view['properties'] = []
#             view['properties'].append(data)
#             break
#     else:
#         st.write("in else statement")
#         view = {
#             "view_name": view_name,
#             "properties": [data]
#         }
...
def update_node():
    ...
    try:
        ...
        new_type = st.selectbox("Select new type for the node", options=["resource", "person", "other"])
...
def create_relation():
    ...
    with node1_col:
        ...
        if node_list and node_name_list:
            selected_index = node_name_list.index(node1_select)
            source_node = node_list[selected_index]['id']
    ...
    with node2_col:
        ...
        if node_list and node_name_list:
            selected_index = node_name_list.index(node2_select)
            target_node = node_list[selected_index]['id']
    ...
    if source_node == target_node:
        ...
        if edge_exists:
            st.error(
                f"A relation of type '{relation_name}' already exists between '{node1_select}' and '{node2_select}'.")
        else:
...
def update_edge():
    ...
    try:
        ...
        new_source = st.selectbox("Select new source node for the edge",
                                  options=[node["id"] for node in st.session_state["node_list"]],
                                  index=[node["id"] for node in st.session_state["node_list"]].index(
...
def visualize_graph():
    ...
    with st.expander("Graph Visualization with Cytoscape", expanded=False):
        ...
        if selected_graph_to_view == 'Main Graph':
            ...
            st.warning("Selected view graph is not available or empty.")
